...
from utils.utils import sanitize_nci_edges


def loss_function(tr_pred, rot_pred, tor_pred, sidechain_pred, 
                  data, t_to_sigma, device, 
                  tr_weight=1, rot_weight=1,tor_weight=1, 
                  backbone_weight=0, sidechain_weight=0, 
...
def debug_nci_batch(data):
    ...
    for i, g in enumerate(graphs):
        if ('ligand', 'nci_cand', 'receptor') not in g.edge_types:
            continue
        edge = g['ligand', 'nci_cand', 'receptor']
        ...
        if bad_lig.any() or bad_rec.any():
            print(f"[BAD edge_index] batch_graph={i} num_lig={num_lig} num_rec={num_rec}")
            print("bad lig idx:", bad_lig.nonzero().flatten().tolist())
            print("bad rec idx:", bad_rec.nonzero().flatten().tolist())
            raise RuntimeError("Invalid NCI edge_index")

        if hasattr(edge, 'edge_type_y'):
            labels = edge.edge_type_y
            if (labels < 0).any() or (labels >= 8).any():
                bad = ((labels < 0) | (labels >= 8)).nonzero().flatten().tolist()
                print(f"[BAD labels] batch_graph={i} bad_idx={bad} "
                      f"min={int(labels.min())} max={int(labels.max())}")
                raise RuntimeError("Invalid NCI labels")
